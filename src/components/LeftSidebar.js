...
async function getConversations(supabase, user) {
  ...
  if (error) {
    toast.error("Failed to retrive conversations. " + error.message);
      ...
      >
        <div className="sticky top-0 flex items-center bg-white pt-2">
          <Link
            className={cn(
              ...
              !queryId && "bg-gray-100 font-semibold text-gray-600"
            )}
            href="/"
          >
          ...
          <button
            className="rounded-full ml-2 flex items-center justify-between p-2 hover:bg-gray-100 text-gray-400 text-lg"
            title="Hide"
        ...
        {conversations?.map((conversation) => (
          <Link
            key={conversation.id}
            className={cn(
              "flex cursor-pointer items-center rouned-md p-3 text-gray-500 hover:bg-gray-50 hover:text-gray-900",
              conversation.id == queryId &&
