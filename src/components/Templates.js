      ...
      <div className="mx-auto max-w-4xl overflow-y-auto w-full">
        ...
        >
          {templates.map((template) => (
            <li
              key={template.slug}
              className="group col-span-1 cursor-pointer divide-y divide-gray-200 rounded-lg border 
              bg-white hover:shadow dak:border-gray-400 dak:bg-transparent"
            >
              <Link href={`/${template.slug}`}>
                <div className="flex h-full w-full flex-col p-5">
                  ...
                  <h3
                    className="mt-2 truncate text-lg font-medium text-gray-900 group-hover:text-blue-500 
                  "
                  >
                    {template.title}
                  </h3>
